...
import type { PageSide, PageOrientation, 
            ContainerSide, ContainerSizeRelativeTo,
            ScaleInput, ImageOptions, TextOptions } from './types'
...
import type { PageSize, AnyPageContainer, 
        TableContainerOptions as TableOptions, DocPathStyle,
        ContainerAlignment, ContainerHAlignment, ContainerVAlignment,
        ContainerPositionLike, ContainerPositionAbs, DocSettings, DocUnits, DocUnitsWithPerc, PercentageString, ValueWithUnitsString, WidthHeightInput, 
        ContainerTableInput, DocData, DocGraphicInputRect, DocGraphicInputCircle, 
        DocGraphicInputOrthoLine,
        ContainerBlock, TitleBlockInput, LabelBlockOptions, DocPipeline
         } from './types'
...
import { isDocUnits, isPercentageString, isValueWithUnitsString, isAnyPageContainer, 
    ...
    isContainerPositionLike, isContainerHAlignment, 
    isContainerVAlignment, isContainerAlignment, 
    isContainerPositionCoordAbs, isPageSize } from './typeguards' // typeguards
...
export class Doc
{
    ...
    get modelerClasses()
    {
        ...
        {
            throw new Error(`Doc::modelerClasses: Cannot get modeler classes. Archiyou modules not set or modeler module not found! Please set archiyou modules with setArchiyou() before using.`);
        }
    ...
    */
                            ...
                            */
                            ...
                            if(!outputs || typeof outputs !== 'object')
                            {
                                ...
                                Object.entries(outputs).forEach(([key, value]) => 
                                {
                                    ...
                                    {
                                         console.warn(`Doc:executePipelines(): Overwriting existing variable "${key}" on execution scope!`); 
                                    }
    ...
    create(name?:string):Doc
    {
        ...
        console.info(`Doc::create(): Created new Document "${this._activeDoc.name}" with default settings [${newDoc.units} - ${newDoc.pageSize} - ${newDoc.pageOrientation}]`)
    ...
    */
     ...
     * 
    */
    table(nameOrData:ContainerTableInput, options?:TableOptions):Doc
    {
        ...
        if(typeof nameOrData === 'string' && !this._archiyou.calc){ throw new Error(`Doc::table: Cannot get table data from Calc module. Calc is not initialized. Use calc.init()`); }
        if(typeof nameOrData === 'string' && !this._archiyou.calc.tables().includes(nameOrData as string)){ { throw new Error(`Doc::table: Cannot get table data from Calc module. No such table: '${nameOrData}'. Available tables: ${this._archiyou.calc.tables().join(',')}`); } }
        ...
        {
            const calcTable = this._archiyou.calc.db.table(nameOrData as string);
    ...
    _getParamSummary():string
    {
        ...
        const params = (this._archiyou as any)?.worker?._activeExecRequest?.script?.params as Record<string,ScriptParamData>;

        if (!params || Object.keys(params).length === 0){ return 'no parameters' }

        const paramValues = (this._archiyou as any)?.worker?._activeExecRequest?.params;
    ...
    _getMetricSummary():string
    {
        ...
        const metrics = Object.values((this._archiyou as any)?.calc?.metrics()); // TODO: publishScript too?
        if (!metrics)
        {
            return 'no metrics'
        }
    ...
    _getVersion():string
    {
        const version = (this._archiyou as any)?.worker?.lastExecutionRequest?.script?.published_as?.version // editor app
            || (this._archiyou as any)?.worker?.lastExecutionRequest?.version // compute worker context:  OcciCadScriptRequest
            || '0'
    ...
    {
        return `${this._getVersion()} at ${(this._archiyou as any)?.worker?.lastExecutionRequest?.createdAtString || new Date().toLocaleString('nl-NL') }`;
    }
    ...
    shapes(shapes:SmartShapeCollection|string, all:boolean=false):Doc
    {
    ...
    {
        console.info(`Doc::toSVG(): Exporting docs to SVG: ${only ? ((Array.isArray(only) && only.length > 1) ? (only as Array<string>).join(', ') : only) : 'all'}`);
        ...
        {
            svgStringsByDocName[doc.name] = await doc.toSVG(this._assetsCache);
        }
    ...
    */
    ...
    /** Merge incoming Document instances with current document 
     *  @d single or collection of Document instance 
    */
    merge(d:Document|Array<Document>|Record<string, Document>, namePrefix:string=''):this
    {
        ...
        if(docsArray.length === 0){ throw new Error(`Doc::merge: Please supply at least one Document to merge!`); }

        docsArray.forEach((doc, i) => 
        {
            if(!(doc instanceof Document)){ console.error(`Doc::merge: Encountered a object of type ${typeof doc} which is not a Document at index ${i}. Skipping it!`)};
            ...
            doc._pages.forEach((page) =>
            {
                ...
                if(!this._activeDoc.pageExists(mergedPageName))
                {
                    ...
                    console.info(`Doc::merge: Merged page "${page.name}" from Document "${mergedDocName}" into active Document "${this._activeDoc.name}"`);
                }
                else {
                    // NOTE: this should not happen! 
                    console.warn(`Doc::merge: Page "${page.name}" already exists in active Document "${this._activeDoc.name}". Skipping it!`);
                }
            });
