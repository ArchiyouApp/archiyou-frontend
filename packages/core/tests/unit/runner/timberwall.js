...
$PARAMS.define('WIDTH', 'number', { label: "Width", group: "main", units: "mm", order: 0, default: 3000, minimum: 1000, maximum: 3000, multipleOf: 1 });
$PARAMS.define('HEIGHT', 'number', { label: "Height", group: "main", units: "mm", order: 1, default: 2500, minimum: 1000, maximum: 2500, multipleOf: 1 });
$PARAMS.define('DEPTH', 'options', { label: "Depth", group: "main", order: 2, default: 120, enum: [120,140,184,235] });
$PARAMS.define('OPENING', 'boolean', { label: "With opening", order: 3, default: false });
$PARAMS.define('INSULATION', 'options', { label: "Insulation", order: 4, default: "hempflax", enum: [] });
$PARAMS.define('TOP_CONNECT', 'boolean', { label: "Double Top", order: 5, default: false });
$PARAMS.define('OPENING_START', 'number', { group: "main", order: 6, default: 1000, minimum: 4000, maximum: 2000, multipleOf: 1 });
$PARAMS.define('OPENING_SILL', 'number', { group: "main", order: 7, default: 500, minimum: 100, maximum: 1500, multipleOf: 1 });
$PARAMS.define('OPENING_WIDTH', 'number', { group: "main", order: 8, default: 600, minimum: 300, maximum: 1500, multipleOf: 1 });
$PARAMS.define('OPENING_HEIGHT', 'number', { group: "main", order: 9, default: 1000, minimum: 300, maximum: 1500, multipleOf: 1 });
$PARAMS.define('BOARDS', 'options', { group: "main", order: 10, default: "none", enum: ["none","OSB9","OSB18"] });
...
BOARDS_OPTIONS_TO_DATA = {
    ...
    'OSB9' : { thickness: 9, kgm3: MATERIALS.OSB.KG_M3, eurm2: 3.14 },
    'OSB18' : { thickness: 18, kgm3: MATERIALS.OSB.KG_M3, eurm2: 5.89 },
...
/*
boards = collection();
if(wall.openingDiagrams.count())
{
  wallOpenings = wall.openingDiagrams.copy();
...
woodLengthM = allWoodBeams.reduce((agg,s) => agg + s.obbox().height(), 0)/1000;
openingsM2 = (wall.openingDiagrams.length) ? wall.openingDiagrams.toArray().reduce((agg,s) => agg + s.bbox().front().area(), 0) / Math.pow(10,6) : 0
...
{
    iso = wall.iso([-1,-1,1]).move($WIDTH*2);
