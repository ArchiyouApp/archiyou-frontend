...
import { isPointLike, isAnyShape, isObjStyle, isBaseStyle } from './typeguards'
import { uuidv4, colorHexToInt } from './utils'
...
export class Obj
{
    ...
    _getColorRGBA():[number,number,number,number]
    {
        ...
        return [r / 255, g / 255, b / 255, 1] as [number, number, number, number];
    }
    ...
    _checkObjStyle(style:ObjStyle):ObjStyle
    {
        ...
        {
            // Normalize colors with Color (meshup)
            if (geomStyle?.color)
            {
                if (typeof geomStyle.color === 'number')
                {
                    // already a numeric color int, keep as-is
                }
                else if (geomStyle.color === 'random')
                {
                    geomStyle.color = Color.random().toInt();
                }
                else if (Color.valid(geomStyle.color))
                {
                    geomStyle.color = new Color(geomStyle.color).toInt();
                }
                else
                {
