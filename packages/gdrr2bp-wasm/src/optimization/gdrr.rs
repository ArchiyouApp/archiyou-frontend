...
impl<'a> GDRR<'a> {
    ...
    pub fn lahc(&'a mut self) {
        ...
        // wasm port: with no separate monitor thread to broadcast Terminate after
        // `max_run_time`, the time budget is enforced directly in the loop below.
        let max_run_time = self.config.max_run_time.map(|s| s as u64).unwrap_or(u64::MAX);
        ...
        while n_iterations < max_rr_iterations && !self.local_sol_collector.terminate()
            && (start_time.elapsed().as_secs() < max_run_time) {
        ...
        timed_thread_println!("{}:\t ({:.2} iter/s, {:.2} acc/s, {} impr)",
                "GDRR finished".bright_magenta(),
                 (n_iterations as f64 / (web_time::Instant::now() - start_time).as_millis() as f64 * 1000.0),
                 (n_accepted as f64 / (web_time::Instant::now() - start_time).as_millis() as f64 * 1000.0),
                n_improved
        );
