export default {
  id: "archiyou/urhousesketch/0.4.0",
  name: "urhousesketch",
  author: "archiyou",
...
function wallXPositionToWorld(wall,x)
{
    ...
    wallBbox = wall.bbox();
    wallLengthAxis = (wallBbox.width() > wallBbox.depth()) ? 'x' : 'y';
    wallDirection = (wallLengthAxis === 'x') ? vector(1,0,0) : vector(0,1,0);
    wallStartWorld = vector(wallBbox.minX()+WALL_THICKNESS/2,wallBbox.minY()+WALL_THICKNESS/2)
                    
    wallLength = (wallLengthAxis === 'x') ? wallBbox.width() : wallBbox.depth();
    wallX = (x >= 0 && x <= 1) 
                ? x*wallLength // perc
                : (x > 1) ? x : wallLength + x;
...
{
    WINDOW_WIDTH = 80;
    WINDOW_HEIGHT_GROUNDFLOOR = 200;
    WINDOW_HEIGHT_TOPFLOOR = 100;
    DISTANCE_BETWEEN_WINDOWS = WINDOW_WIDTH*1.0;
    WINDOW_RANDOMNESS = DISTANCE_BETWEEN_WINDOWS*1.1;

    wallBbox = wall.bbox();
    wallLength = (wallBbox.width() > wallBbox.depth()) ? wallBbox.width() : wallBbox.depth();
    wallRotZ = (wallBbox.width() > wallBbox.depth()) ? 0 : 90;
    numCols =  Math.floor((wallLength - 2*WALL_THICKNESS) / (WINDOW_WIDTH+DISTANCE_BETWEEN_WINDOWS));
    numFloors = (HEIGHT > 500) ? 2 : 1;
    startLeft = (wallLength - 2*WALL_THICKNESS)
    ...
    for(let fl = 0; fl < numFloors; fl++)
    {
        ...
        {
            let windowFrame = makeOpeningFrame(
...
{
    print(f);
    sideFacadeWall = FACADE_TO_WALL[f].wall;
...
print(layer('roof').shapes().getShapesByTypes(['Edge','Wire']).hide());
...
[
    ...
    ['Gutter height max', '', Math.round([roofLineOutsideOverhangsChecked.start().z,roofLineOutsideOverhangsChecked.end().z].sort()[1]), 'cm'],
    ['Gutter height min', '', Math.round(roofLineOutsideOverhangsChecked.bbox().minZ()), 'cm'],
    //['Ground floor area', 'gross', Math.round((WIDTH*DEPTH)*1e-4), 'm2' ],
    //['Ground floor area', 'net', Math.round(((WIDTH-2*WALL_THICKNESS)*(DEPTH-2*WALL_THICKNESS))*1e-4), 'm2' ],   
    ['Number of levels', 'including groundfloor', (numFloors), '#'],
],
...
calc.table(
    ...
    [
        ...
        ['Total floor area', 'gross, estimation', Math.round(numFloors*(WIDTH*DEPTH)*1e-4), 'm2' ],
        ['Total floor area', 'net, estimation', Math.round(numFloors*((WIDTH-2*WALL_THICKNESS)*(DEPTH-2*WALL_THICKNESS))*1e-4), 'm2' ],
        ['Total wall length', '', Math.round((2*WIDTH+2*DEPTH)*1e-2), 'm'],
        ['Facade surface', 'estimation without openings', 
            Math.round((wallFrontFace.area()*2 + wallLeftLine._extruded(DEPTH).area()+wallRightLine._extruded(DEPTH).area())*1e-4),
            'm2'
        ],
        ['Roof surface', 'top surface', Math.round((roofLineOutsideOverhangsChecked._extruded(DEPTH).area()*1e-4)), 'm2']
    ],
