...
export class Db
{
    ...
    _archiyou:ArchiyouModules; // bind to Archiyou modules to d data of Shapes and Scene
    _tables:Object = {}; // { name : Table }
    
    shapes:Table; // all Shapes in the current model (see _archiyou)
    objects:Table; // all Obj containing Shapes in the Scene ( through _archiyou)

    constructor(archiyou:ArchiyouModules)
    {
        this._archiyou = archiyou;

        this.init(); // try to init immediately. Maybe there are not Shapes in Archiyou instance. When there are use init()
    }
    ...
    setupTables()
    {
        ...
        // TODO: AFTER REFACTOR - generateObjsData not yet implemented
    ...
    generateObjsData():Array<Object> // TODO: make output Object an Interface?
    {
        if(!this._archiyou.modeler)
        {
            console.error(`Db::generateObjsData: Cannot get Objs without an instance of Modeler. Please supply it in constructor!`);
            return [];
        }
        return this._archiyou.modeler.allObjs().map(obj => obj.toData());
    }
    */
