...
import type { SVGtoPDFtransform, PageData, ContainerData, 
                DocPathStyle, PDFLinePath } from './types'


import { convertValueFromToUnit, mmToPoints } from './utils'

// TODO: move to a constants file (was in removed DocUtils.ts)
const DOC_DIMENSION_LINES_TEXT_HEIGHT = 3.5; // mm
// TODO: implement properly (was in removed DocUtils.ts)
function filterNestedObjects(obj: any, predicate: (node: any) => boolean, options?: Record<string, any>): Array<any> { return []; }


export class ViewSVGManager
{
    //// SETTINGS ////
    PATH_BASE_STYLE = {
            strokeColor: '#000000', 
    ...
    constructor(view?:ContainerData)
    {
        if(view)
        {
            ...
            if(!r)
            {
                console.error(`ViewSVGManager: No valid SVG data found in view with name "${view.name}"!`)
            }
        }
    }
    ...
    setViewBox()
    {
        if(this._svgXML && this._svgXML?.svg?.viewBox && this._svgXML?.svg?.bbox)
        {
            ...
            console.error(`ViewSVGManager: Could not set viewBox in SVG! Check if SVG could be parsed!`)
        }
    }
    ...
    toPDFDocTransform(pdfExporter:PDFExporter, view:ContainerData, page:PageData): SVGtoPDFtransform
    {
        ...
        {
            console.error('ViewSVGManager:toPDFDocTransform: Error occurred while transforming SVG to PDF:', e);
        }
    }
    ...
    */
    toPDFDocShapePaths(pdfExporter:PDFExporter, view:ContainerData, page:PageData):Array<PDFLinePath>
    {
        ...
        {
            console.warn('ViewSVGManager:toPDFDocShapePaths: No paths found. Returned empty array.')
        }
        else
        {
    ...
    */
    _svgPathClassesToPDFPathStyle(svgPathNode:any):DocPathStyle|null
    {
        ...
        classes.forEach( className => 
        {
            ...
            if(!classStyle)
            {
                console.warn(`ViewSVGManager::_svgPathClassesToPDFPathStyle: Encountered unknown style: ${className}`)
            }
            else {
    ...
    drawDimLinesToPDF(pdfExporter:PDFExporter)
    {
        const DIMLINE_CLASS = 'dimensionline';
        
        if(!this._svgXML){ throw new Error(`ViewSVGManager:drawDimLinesToPDF: Please parse SVG data first!`); }
        if(!pdfExporter){ throw new Error(`ViewSVGManager:drawDimLinesToPDF: Please supply a PDFExporter instance!`); }
    ...
    */
    _drawDimLineLine(dimLineNode:any, pdfExporter:PDFExporter):Array<number>
    {
        const dimLineCoords = this._svgDimLineParseLine(dimLineNode);
    ...
    */
    _drawDimLineArrows(dimLineNode:any, pdfExporter:PDFExporter)
    {
    ...
    _drawDimLineText(dimLineNode:any, pdfExporter:PDFExporter)
    {
        // TODO: implement using XMLparser
        /*
        ...
        if (this._svgDimLineIsSmall(dimLineNode))
        {
            ...
            if(!offsetVec)
            { 
                console.warn(`ViewSVGManager::_drawDimLineText(): Can't get offset vector. Skipped offsetting small dimension line`); 
            }
            else 
            {
    ...
    */
    _drawSVGPathToPDF(d:string, pdfExporter:PDFExporter, localTransforms?:Record<string, any>, styles?:Record<string, any>)
    {
        ...
        if(!doc){
            throw new Error(`ViewSVGManager::_drawSVGPathToPDF: Please supply activePDFDoc (jsPDF) instance`)
        }
        ...
        pathCommands.forEach(cmd => 
        {
            if(drawContext[PATH_CMD_TO_JSPDF[cmd.code]]) // basic protection
            {
                ...
                console.error(`ViewSVGManager::_drawSVGPathToPDF: Unknown/unmapped SVG path command: "${cmd.code}"`)
                return null;
            }
        })    
    ...
    {
        if(!transform){ throw new Error(`ViewSVGManager:_svgCoordToPDFcoord(): Please supply a SVGtoPDFtransform object`)}
