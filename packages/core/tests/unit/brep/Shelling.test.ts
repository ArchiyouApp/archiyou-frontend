...
{
    // Solid Box
    expect(new brep.Solid().makeBox(100).shelled(5).faces().length).toEqual(32);
    expect(new brep.Solid().makeBox(100).shelled(5, 'F||top').faces().length).toEqual(23); // top off
    expect(new brep.Solid().makeBox(100).shelled(-5).faces().length).toEqual(12);
    expect(new brep.Solid().makeBox(100).shelled(-5, 'F||top').faces().length).toEqual(11); // top off
    expect(new brep.Solid().makeBox(100).shelled(5, 'F||top', 'intersection').faces().length).toEqual(11); // top off, with straight join type
    // Solid Sphere
    expect(new brep.Solid().makeSphere(100).shelled(5).faces().length).toEqual(2);
    expect(new brep.Solid().makeSphere(100).shelled(-5).faces().length).toEqual(2);
    // Face
    expect(new brep.Face([0,0,0],[100,100,100],[0,100,0]).shelled(5).faces().length).toEqual(5);
    // Shell
